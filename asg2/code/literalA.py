...
def calculate_variance(x, mean):
	summ = 0
	for item in x:
		summ = summ + pow(item - mean, 2)
	return float(summ)/len(x)
...
class GaussianClassification1D:
	def fit(_self,x,y,labels):
		...
		for k, label in labels.items():
			mean[label] = calculate_mean(x_temp[label])
			variance[label] = calculate_variance(x_temp[label], mean[label])
			prior_class[label] = calculate_prior_class(x_temp[label],len(x))

		# Storing the data as a class variable	
		_self.means = mean
		_self.variances = variance
		_self.prior_classes = prior_class

	def get_membership(_self,x,mean,variance,prior_class):
		'''
		...
		'''
		numerator = np.power(x - mean,2)
		denominator = 2 * (pow(variance,2))
		division = np.divide(numerator,denominator, dtype=float)
		return (division * -1 ) + math.log10(prior_class) - math.log10(variance)
	...
	def predict(_self,x, labels):
		...
		for k, label in labels.items():
			membership = _self.get_membership(x,_self.means[label],_self.variances[label],_self.prior_classes[label])
